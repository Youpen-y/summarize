...
- extracted `env`, `read`, `model`, and section parsers
- remaining: move exported types out, then trim remaining top-level scalar parsing
...
- extracted slide state store and added direct waiter/done tests
- remaining: terminal renderer split, then stream-machine extraction
...
- extracted subprocess execution, payload parsing, media extraction, and shared tweet types
